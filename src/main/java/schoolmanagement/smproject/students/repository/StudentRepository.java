...
public class StudentRepository implements IStudentRepository {
    ...
    public Student save(Student student) {
        String sql = "INSERT INTO students (first_name, last_name, email, phone, date_of_birth, gender, address, enrollment_date, grade_level, classroom, status, primary_parent_id, secondary_parent_id, emergency_contact_name, emergency_contact_phone, emergency_contact_relationship) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public Student update(Student student) {
        String sql = "UPDATE students SET first_name = ?, last_name = ?, email = ?, phone = ?, date_of_birth = ?, gender = ?, address = ?, enrollment_date = ?, grade_level = ?, classroom = ?, status = ?, primary_parent_id = ?, secondary_parent_id = ?, emergency_contact_name = ?, emergency_contact_phone = ?, emergency_contact_relationship = ? WHERE id = ?";
        try (Connection conn = DatabaseConnection.getConnection();
                PreparedStatement stmt = conn.prepareStatement(sql)) {

            mapStudentToStatement(stmt, student, true);
            stmt.setInt(17, student.getId());
            stmt.executeUpdate();
            return student;
    ...
    public Optional<Student> findById(int id) {
        String sql = "SELECT s.*, p1.id AS p1_id, p1.first_name AS p1_fn, p1.last_name AS p1_ln, p1.email AS p1_em, p1.phone AS p1_ph, p1.phone_alternate AS p1_pa, p1.relationship AS p1_rel, p1.occupation AS p1_occ, p1.address AS p1_addr, p1.is_primary_contact AS p1_ip, p2.id AS p2_id, p2.first_name AS p2_fn, p2.last_name AS p2_ln, p2.email AS p2_em, p2.phone AS p2_ph, p2.phone_alternate AS p2_pa, p2.relationship AS p2_rel, p2.occupation AS p2_occ, p2.address AS p2_addr, p2.is_primary_contact AS p2_ip FROM students s LEFT JOIN parents p1 ON s.primary_parent_id = p1.id LEFT JOIN parents p2 ON s.secondary_parent_id = p2.id WHERE s.id = ?";
    ...
    public List<Student> findAll() {
        String sql = "SELECT s.*, " +
                ...
                "p1.email AS p1_em, p1.phone AS p1_ph, p1.phone_alternate AS p1_pa, " +
                "p1.relationship AS p1_rel, p1.occupation AS p1_occ, " +
                "p1.address AS p1_addr, p1.is_primary_contact AS p1_ip, " +
                "p2.id AS p2_id, p2.first_name AS p2_fn, p2.last_name AS p2_ln, " +
                "p2.email AS p2_em, p2.phone AS p2_ph, p2.phone_alternate AS p2_pa, " +
                "p2.relationship AS p2_rel, p2.occupation AS p2_occ, " +
                "p2.address AS p2_addr, p2.is_primary_contact AS p2_ip " +
                "FROM students s " +
    ...
    private void mapStudentToStatement(PreparedStatement stmt, Student student, boolean includeId) {
        try {
            ...
            stmt.setString(i++, student.getClassroom() != null ? student.getClassroom() : " ");
            ...
            stmt.setString(i++,
                    student.getEmergencyContactRelationship() != null ? student.getEmergencyContactRelationship()
                            : " ");
        } catch (SQLException e) {
    ...
    private Optional<Student> executeQuerySingle(String sql, StatementSetter setter, boolean joinParents) {
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next())
                    return Optional.of(mapResultSetToStudent(rs, joinParents));
            }
        } catch (SQLException e) {
    ...
    public List<Object[]> getEnrollmentByMonth(String academicYear) {
        String sql = "SELECT MONTHNAME(enrollment_date) as month, COUNT(*) as count FROM students WHERE YEAR(enrollment_date) = YEAR(CURDATE()) GROUP BY MONTH(enrollment_date) ORDER BY MONTH(enrollment_date)";
        List<Object[]> results = new ArrayList<>();
        try (Connection conn = DatabaseConnection.getConnection();
                PreparedStatement stmt = conn.prepareStatement(sql);
                ResultSet rs = stmt.executeQuery()) {
            while (rs.next()) {
                results.add(new Object[] { rs.getString("month"), rs.getInt("count") });
            }
        } catch (SQLException e) {
